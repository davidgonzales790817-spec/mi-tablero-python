...
for k, v in {"df": None, "df_procesado": None, "columnas": {},
             "cols_devengado": [], "col_generica": None,
             "fecha_corte": date.today()}.items():
...
with t1:
    ...
    try:
        grid_kpis([
            {"titulo": "PIM",
             "valor": ind["ejecucion"]["pim_total"], "formato": "soles"},
            {"titulo": "Certificado",
             "valor": ind["ejecucion"]["certificado_total"], "formato": "soles",
             "progreso": ind["ejecucion"]["pct_certificado"], "target": 33,
             "estado": color_por_avance(ind["ejecucion"]["pct_certificado"])},
            {"titulo": "Devengado",
             "valor": ind["ejecucion"]["devengado_total"], "formato": "soles",
             "progreso": ind["ejecucion"]["pct_avance_financiero"], "target": 33,
             "estado": color_por_avance(ind["ejecucion"]["pct_avance_financiero"]),
             "subtitulo": "Indicador oficial MEF"},
            {"titulo": "Forecast cierre",
             "valor": ind["proyecciones"]["proyeccion_pct"], "formato": "porcentaje",
                ...
                for gen in df[col_generica].dropna().unique()[:6]:
                    ...
                    st.markdown(f"""<div style="margin-bottom:6px;">
                      <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:2px;">
                        <span>{str(gen)[:35]}</span><b>{pct:.1f}%</b></div>
